...
1. `docs/worklog.md` … 直近の作業ログ（新しい順）。**まずここで最新の状況を把握する。**
...
ユーザーが以下の形式で指示したら、**自動で作業ログへ追記して push** すること。
セッションが変わった際の引き継ぎ材料になるため、要点を漏らさず記録する。

### トリガー
- `作業ログに記録：<本文>`（または「作業ログに残して」「ログに記録して」等の同義の依頼）
  … 既定ファイル `docs/worklog.md` に追記。

### 追記の手順
1. 対象ファイル（既定は `docs/worklog.md`）を開く。
...
3. `git add` → コミット（例メッセージ: `docs: 作業ログを追記`）→ 現在の作業ブランチへ push。
   他の変更と同様の運用が自然な場合は ff-merge で main にも反映する。
