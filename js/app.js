...
(function () {
  ...
  function standaloneHTML() {
    ...
    return '<!DOCTYPE html>\n<html lang="ja"><head><meta charset="UTF-8">' +
      ...
      new Date().toLocaleString('ja-JP') + '</footer>' +
      '<script>document.addEventListener("error",function(e){var t=e.target;' +
      'if(t&&t.tagName==="IMG"&&t.hasAttribute("data-hide-on-error")){var f=t.closest("figure");if(f)f.style.display="none";}},true);<\/script>' +
      '</body></html>';
  }
  ...
  /* ============ イベント登録 ============ */
  // レポート内の壊れた画像を隠す（インラインonerrorを使わずCSPと両立）。
  // errorイベントはバブリングしないためキャプチャで捕捉する。
